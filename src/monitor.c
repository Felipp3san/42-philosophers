...
/*   Updated: 2025/07/26 18:07:08 by fde-alme         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

static void		verify(t_table *table, t_philo *philo);
...
void	*monitor(void *data)
{
	...
	table = (t_table *) data;
	log_debug(table, GREEN, "Monitor started!", NULL);
...
static void	verify(t_table *table, t_philo *philo)
{
	...
	{
		log_debug(table, RED, "Dead philosopher found, stopping threads...", NULL);
		set_bool(&table->table_mutex, &table->finished, TRUE);
		write_status(philo, DEAD);
	}
	else if (all_philos_full(table))
	{
		log_debug(table, GREEN, "All philos are full, stopping simulation...", NULL);
		set_bool(&table->table_mutex, &table->finished, TRUE);
	}
}
